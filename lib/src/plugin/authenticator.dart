...
class DummyAuthenticator extends Authenticator {
  final _userInfos = HashMap<String, _DummyUserInfo>();

  ///Adds a user.
  void addUser(String username, String password, Iterable<String> roles) {
    _userInfos[username] = _DummyUserInfo(SimpleUser(username, roles), password);
  }
...
/// Notice that it is used only for demonstration. Your implementation
...
class SimpleUser {
  ...
  : _roles = roles is Set<String> ? roles: Set<String>.from(roles);

...
abstract class Security<User> {
  ...
  /// * [rememberMe] - provides the plugin implementing *remember-me*.
  ...
  => _Security(authenticator, accessControl,
      redirector ?? Redirector(), rememberMe,
      rememberUri ?? RememberUri(),
      onLogin, onLogout);

  /// The filter used to configure Stream server's filter mapping.
  ///
  /// It must be specified in `filterMapping` when instantiating [StreamServer]
  ...
  /// auto sign-in).
  ///
  /// * [username] - specifies the user name. If not specified, [username]
  ...
  /// to the logout action, `/s_logout`, as described in [Security]:
  ///
  ///     "/s_logout": security.logout,
  ///
...
abstract class Authenticator<User> {
  ...
  /// * Returns a [Future] carrying the data you'd like to preserve in the
  /// new session after logout. The Future may yield null to preserve nothing.
  Future<Map?> logout(HttpConnect connect, User user) async => null;
